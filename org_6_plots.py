...
for p in range(len(ReList)):
    for m in range(ens_size):
        ...
        err_cae_pred[p, m] = np.mean(
            (tfom[p, :, :, :] - t_lstm_cae[p, m, :, :, :]) ** 2
        )
        err_nlpod_rec[p, m] = np.mean(
            (tfom[p, :, :, :] - t_ae_nlpod[p, m, :, :, :]) ** 2
        )
        err_nlpod_pred[p, m] = np.mean(
            (tfom[p, :, :, :] - t_lstm_nlpod[p, m, :, :, :]) ** 2
        )
...
plt.fill_between(
    index[:ncut0], tric[:ncut0], y2=tric[0], alpha=0.6, color="orange"
)  # ,
...
plt.text(
    1.1,
    78,
    r"$" + str(np.round(tric[ncut0 - 1], decimals=2)) + "\%$",
    va="center",
    fontsize=18,
)
plt.text(
    5.1,
    101,
    r"$" + str(np.round(tric[ncut1 - 1], decimals=2)) + "\%$",
    va="center",
    fontsize=18,
)
...
for j, ind in enumerate([150, 200]):
    ...
    cs = axs[3][j].contour(
        X,
        Y,
        np.mean(t_lstm_cae[ReList.index(Re), :, :, :, ind], axis=0),
        v,
        cmap=colormap,
        linewidths=1,
        extend="both",
    ...
    cs = axs[4][j].contour(
        X,
        Y,
        np.mean(t_lstm_nlpod[ReList.index(Re), :, :, :, ind], axis=0),
        v,
        cmap=colormap,
        linewidths=1,
        extend="both",
