...
def AE_trainDNN(
    ...
    if not xtrain.shape[0] == ytrain.shape[0]:
        print(
            "Size mismatch between xtrain and ytrain! \
              The passed xtrain has {0} samples while ytrain have {1} samples.".format(
    ...
    if not xvalid.shape[0] == yvalid.shape[0]:
        print(
            "Size mismatch between xvalid and yvalid! \
              The passed xvalid has {0} samples while yvalid have {1} samples.".format(
                xvalid.shape[0], yvalid.shape[0]
            )
        )
        raise AssertionError
    ...
    autoencoder = Model(
        inputs=[input_data, pg_input],
        outputs=decoder(inputs=[encoder(input_data), pg_input]),
        name="autoencoder",
    )
    ...
    history = autoencoder.fit(
        ...
        validation_data=([xvalid, xpvalid], yvalid),
        verbose=verbose,
    )
...
def lstm_scaling(
    ...
    if not q == lookback:
        print(
            "Size mismatch for xtrain! \
              The passed features has a lookback of {0} while expected {1}.".format(
                q, lookback
            )
        )
        raise AssertionError
    ...
    if split:
        ...
        slice_train, slice_valid = train_test_split(
            slice_list, test_size=split_size, shuffle=shuffle
        )
    else:
    ...
    return (
        xtrain_sc[slice_train],
        xtrain_sc[slice_valid],
        xscaler,
        ytrain_sc[slice_train],
        ytrain_sc[slice_valid],
        yscaler,
    )
...
def create_training_data_lstm(lookback, features, labels):
    ...
    if not features.shape[0] == labels.shape[0]:
        print(
            "Size mismatch between features and labels! \
              The passed features has {0} samples while labels have {1} samples.".format(
    ...
    ytrain = [
        (labels[i, :] - labels[i - 1, :]) / dtrom for i in range(lookback, n_samples)
    ]
    ytrain = np.array(ytrain)
    if not (ytrain.shape[0] == n_samples - lookback) and (ytrain.shape[1] == nd_out):
        print(
            "Inconsistent size for ytrain. This should never happen, please check here!"
        )
        raise AssertionError
...
def create_parametric_training_data_lstm(lookback, parameters, features, labels):
    ...
    if not features.shape[0] == labels.shape[0]:
        print(
            "Size mismatch between features and labels! \
              The passed features has {0} parameter values while labels have {1} values.".format(
                features.shape[0], labels.shape[0]
            )
        )
        raise AssertionError
...
zdata = np.zeros([len(bcList), (nx) * (ny), ns + 1])  # field
...
Rmin = -1
Rmax = 1
for j, bc in enumerate(bcTrain):
    tmp = zcoeff[bcList.index(bc), :nr, ::inc]

    X_std = (bc - bcTrain[0]) / (bcTrain[-1] - bcTrain[0])
    Res = X_std * (Rmax - Rmin) + Rmin
    if j == 0:
        zcoeff_train = tmp.T
        param_train = Res * np.ones([tmp.shape[1], 1])
    else:
        zcoeff_train = np.vstack((zcoeff_train, tmp.T))
...
slice_list = np.arange(0, zcoeff_train.shape[0], 1)
slice_train, slice_valid = train_test_split(slice_list, test_size=0.2, shuffle=True)

x_train, x_valid = zcoeff_train[slice_train], zcoeff_train[slice_valid]
xp_train, xp_valid = param_train[slice_train], param_train[slice_valid]
y_train, y_valid = zcoeff_train[slice_train], zcoeff_train[slice_valid]
...
for m in range(ens_size):
    ...
    # train AE seed_lib[m]
    aeconfigs = dict(
        seed=seed_lib[m], epochs=250, batch_size=32, activation="tanh", verbose=1
    )
    (
        encoder,
        decoder,
...
out_results = {
    ...
    "ae_valid_losses": ae_valid_losses,
...
for m in range(ens_size):
    ...
    latent_coeff = []
    encoder = encoders[m]
    for p, Re in enumerate(bcTrain):
        dataPOD = zcoeff[bcList.index(Re), :nr, ::inc].T
    ...
    xt_lstm, yt_lstm = create_parametric_training_data_lstm(
        lookback, parameters=bcTrain, features=latent_coeff, labels=latent_coeff
    )

    xtrain_lstm, xvalid_lstm, xscaler, ytrain_lstm, yvalid_lstm, yscaler = lstm_scaling(
        lookback,
        xt_lstm,
        yt_lstm,
        low=-1,
    ...
    lstm_model, train_loss, valid_loss, training_time_lstm = lstm_train(
        xtrain=xtrain_lstm,
        ytrain=ytrain_lstm,
        xvalid=xvalid_lstm,
        yvalid=yvalid_lstm,
...
for bc in bcList:
    ...
    dataPOD = zcoeff[bcList.index(bc), :nr, ::inc].T
    ...
    for m in range(ens_size):
        ...
        for i in range(lookback, ns + 1):
            testing_time_init = timer.time()
            lstm_coeff[m, i, :] = lstm_coeff[m, i - 1, :] + dtrom * lstm_predict(
                lstm_model, xscaler, yscaler, xtest, lookback, scaling=scaling
            )
            testing_time += timer.time() - testing_time_init
    ...
    Phiz = zbasis[:, :nr]
    z_fom = zdata[bcList.index(bc), :, :]
    z_pod2 = PODrec_svd(
        (dataPOD[:, :latent_dim]).T, Phiz[:, :latent_dim]
    ) + zmean.reshape(-1, 1)
    z_pod72 = PODrec_svd((dataPOD[:, :nr]).T, Phiz[:, :nr]) + zmean.reshape(-1, 1)

    z_ae = np.zeros([ens_size, (nx) * (ny), ns + 1])
    z_lstm = np.zeros([ens_size, (nx) * (ny), ns + 1])
    for m in range(ens_size):
        z_ae[m, :, :] = PODrec_svd(decoded_ae[m, :, :].T, Phiz) + zmean.reshape(-1, 1)
        z_lstm[m, :, :] = PODrec_svd(decoded_lstm[m, :, :].T, Phiz) + zmean.reshape(
            -1, 1
        )
    ...
    np.savez(
        filename,
        latent_ae=latent_coeff,
        latent_lstm=lstm_coeff,
        b_ae=decoded_ae,
        b_lstm=decoded_lstm,
        z_ae=z_ae,
        z_lstm=z_lstm,
