...
def AE_trainCNN(
    ...
    x = Dropout(0.2)(x)
    x = Dense(
        np.prod(volumeSize[1:]), activation=activation, kernel_initializer=initializer
    )(x)
    ...
    autoencoder = Model(
        inputs=[input_img, pg_input],
        outputs=decoder(inputs=[encoder(input_img), pg_input]),
        name="autoencoder",
    )
    ...
    # compile model
    print("Autoencoder > compile model")
    autoencoder.compile(optimizer="adam", loss="mse")  # ,metrics=['mse'])

    # fit the model (optimize weights and biases)
    print("Autoencoder > fit the model model")
...
#%% reading data
zdata = np.zeros([len(bcList), ns + 1, nx, ny, 1])  # temperature
...
for j, bc in enumerate(bcList):
    ...
    for n in range(nstart, nend + 1, nstep):
        ...
        zdata[j, ii, :, :, 0] = np.copy(z[:, :])  # np.copy(z[:-1, :-1])
        ii = ii + 1
...
for m in range(ens_size):
    ...
    aeconfigs = dict(
        seed=seed_lib[m], epochs=250, batch_size=32, activation="relu", verbose=0
    )
    (
        encoder,
        decoder,
...
for m in range(ens_size):
    ...
    encoder = encoders[m]
    for j, bc in enumerate(bcTrain):
        datafom = zdata[bcList.index(bc), ::inc, :, :, :]
...
for bc in bcList:
    ...
    data_test = zdata[bcList.index(bc), :, :, :]
