...
def jacobian(nx, ny, dx, dy, q, s):
    ...
    # Arakawa 1:nx,1:ny
    j1 = gg * (
        (q[2 : nx + 1, 1:ny] - q[0 : nx - 1, 1:ny])
        * (s[1:nx, 2 : ny + 1] - s[1:nx, 0 : ny - 1])
        - (q[1:nx, 2 : ny + 1] - q[1:nx, 0 : ny - 1])
        * (s[2 : nx + 1, 1:ny] - s[0 : nx - 1, 1:ny])
    )

    j2 = gg * (
        q[2 : nx + 1, 1:ny] * (s[2 : nx + 1, 2 : ny + 1] - s[2 : nx + 1, 0 : ny - 1])
    ...
    j3 = gg * (
        q[2 : nx + 1, 2 : ny + 1] * (s[1:nx, 2 : ny + 1] - s[2 : nx + 1, 1:ny])
...
def laplacian(nx, ny, dx, dy, w):
    ...
    bb = 1.0 / (dy * dy)
    lap = aa * (
        w[2 : nx + 1, 1:ny] - 2.0 * w[1:nx, 1:ny] + w[0 : nx - 1, 1:ny]
...
for n in range(ns):
    ...
    testing_time_init = timer.time()
    aGP[:, n + 1], bGP[:, n + 1] = RK3(
        nr, GROMrhs, cterm, lterm, clterm, nlterm, aGP[:, n], bGP[:, n], dtrom
    )
    testing_time += timer.time() - testing_time_init
...
filename = (
    "./Results/" + folder + "/GP_data_Re=" + str(int(Re)) + "_nr=" + str(nr) + ".npz"
)
np.savez(filename, aGP=aGP, bGP=bGP, wGP=wGP, sGP=sGP, tGP=tGP)
